...
class GameScene(SceneBase):
    ...
    def render(self, win):
        ...
        win.fill(BLACK)
        # draw the map
        self.map.draw_cropped_map(win, self.player_x_pos, self.player_y_pos, \
...
class Game2PlayersScene(SceneBase):
    def __init__(self, kw={}):
        """Initialization of the scene."""
        SceneBase.__init__(self, kw)
        self.map = Map(os.path.join("maps", self.kw.get("map_file")))
    ...
    def render(self, win):
        ...
        self.map.draw_cropped_map(win, self.player_1_x_pos, self.player_1_y_pos, \
                            screen_pos_x=0, screen_pos_y=0, screen_width=WIN_WIDTH//2 - margin_between_players//2, screen_height=WIN_HEIGHT)
        self.map.draw_cropped_map(win, self.player_2_x_pos, self.player_2_y_pos, \
